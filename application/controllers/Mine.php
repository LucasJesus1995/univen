...
class Mine extends CI_Controller
{
   ...
   public function login()
    ...
    public function painel()
    {
        ...
        $data['servicos'] = $this->Conecte_model->getLastServicos($this->session->userdata('cliente_id'));
    ...
    function chamados()
    {
        ...
        $data['menuServicos'] = 'servicos';
        $this->load->library('pagination');
        
        
        $config['base_url'] = base_url().'index.php/mine/chamados/';
        $config['total_rows'] = $this->Conecte_model->count('servicos', $this->session->userdata('cliente_id'));
        $config['per_page'] = 10;
        $config['next_link'] = 'Próxima';
        $config['prev_link'] = 'Anterior';
        $config['full_tag_open'] = '<div class="pagination alternate"><ul>';
        ...
        $data['results'] = $this->Conecte_model->getServicos('servicos', '*', '', $config['per_page'], $this->uri->segment(3), '', '', $this->session->userdata('cliente_id'));
       
        $data['output'] = 'conecte/chamados';
        $this->load->view('conecte/template', $data);
    ...
    public function visualizarChamados($id = null)
    {
        ...
        $data['result'] = $this->servicos_model->getById($this->uri->segment(3));
        

        if ($data['result']->clientes_id != $this->session->userdata('cliente_id')) {
            $this->session->set_flashdata('error', 'Este chamado não pertence ao cliente logado.');
            redirect('mine/painel');
        }

        $data['output'] = 'conecte/visualizar_chamado';
        $this->load->view('conecte/template', $data);
    ...
    public function adicionarOs()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'usuarios_id' => '1', //set_value('idUsuario'),
            ...
            if (is_numeric($id = $this->Conecte_model->add('os', $data, true))) {
                $this->session->set_flashdata('success', 'Proposta adicionada com sucesso! Seus dados do cadastro e descrição do produto foram encaminhados e estão em análise. Você receberá a confirmação no prazo de até 24 horas. Por favor, aguarde a resposta por e-mail!');
    ...
    public function detalhesOs($id = null)
    {
     ...
     // Cadastro de Chamado pelo cliente
    public function adicionarChamado()
    {        
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (is_numeric($id = $this->Conecte_model->add('servicos', $data, true))) {
                $this->session->set_flashdata('success', 'Chamado adicionado com sucesso!');
        ...
        $data['output'] = 'conecte/adicionarChamado';
        $this->load->view('conecte/template', $data);
    }
