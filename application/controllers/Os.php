...
class Os extends CI_Controller 
{
    ...
    function gerenciar()
    {
        ...
        $this->data['results'] = $this->os_model->getOs('os', 'idOs,dataInicial,dataFinal,clientes_id,rep,garantia,descricaoProduto,status,observacoes,origemProposta', $where_array, $config['per_page'], $this->uri->segment(3));
    ...
    function adicionar()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para adicionar Proposta');
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            $data = array(
                ...
                'origemProposta' => set_value('origemProposta'),
                'rep' => set_value('rep'),
                ...
                'tipoFaturamento' => set_value('faturamento'),
                'frete' => set_value('frete'),
                'prazoPg' => set_value('prazo'),
            ...
            if (is_numeric($id = $this->os_model->add('os', $data, true))) {
                ...
                $Rend = array('os_id' => $id, 'cliente_id' => $this->input->post('clientes_id'));
                $this->db->insert('endereco_cobranca',$Rend);

                $assinante = "UPDATE clientes set assinanteConecta = ? WHERE idClientes = ?";
                $this->db->query($assinante, array('1', $this->input->post('clientes_id')));


                $this->session->set_flashdata('success', 'Proposta adicionada com sucesso, você pode adicionar produtos a essa Proposta na abas de "Produtos"!');
        ...
        $this->data['representante'] = $this->os_model->getRepresentante($this->uri->segment(3));
    ...
    function editar()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'eOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para editar Proposta');
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            $data = array(
                ...
                'tipoFaturamento' => $this->input->post('faturamento'),
                'frete' => $this->input->post('frete'),
                'prazoPg' => $this->input->post('prazo'),
                'descricaoProduto' => $this->input->post('descricaoProduto'),
                'status' => $this->input->post('status'),
                'observacoes' => $this->input->post('observacoes'),
                'usuarios_id' => $this->input->post('usuarios_id'),
                'clientes_id' => $this->input->post('clientes_id'),
                'origemProposta' => $this->input->post('origemProposta'),
            ...
            if ($this->os_model->edit('os', $data, 'idOs', $this->input->post('idOs')) == true) {
                if ($this->input->post('status') == '5' or $this->input->post('status') == '6') {
                    $sql = "UPDATE clientes set assinanteConecta = ? WHERE idClientes = ?";
                    $this->db->query($sql, array('0', $this->input->post('clientes_id')));
                }
                if ($this->input->post('status') == '4') {
                    $pedido = array('protocolo' => $this->input->post('idOs'), 'cliente_id' => $this->input->post('clientes_id'), 'statusPedidos' => '4', 'dataPedido' => date('Y/m/d'));
        ...
        $this->data['cobranca'] = $this->os_model->getCobranca($this->uri->segment(3));
        $this->data['representante'] = $this->os_model->getRepresentante($this->uri->segment(3));
    ...
    public function visualizar()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar Proposta');
            redirect(base_url());
        }
    ...
    function excluir()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'dOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para excluir Proposta');
            redirect(base_url());
        }
        
        $id =  $this->input->post('id');
        $assinante =  $this->input->post('assinante');
        ...
        $this->db->where('os_id', $id);
        $this->db->delete('endereco_cobranca');

        $this->db->where('os_id', $id);
        $this->db->delete('anexos');

        $sql = "UPDATE clientes set assinanteConecta = ? WHERE idClientes = ?";
                $this->db->query($sql, array('0', $assinante));
    ...
    public function autoCompleteRepresentante()
    {

        if (isset($_GET['term'])) {
            $q = strtolower($_GET['term']);
            $this->os_model->autoCompleteRepresentante($q);
        }
...
function editarEndEntrega()
    {
            ...
            if ($this->os_model->edit('clientes', $data, 'idClientes', $this->input->post('idClientes')) == true) {
                $this->session->set_flashdata('success', 'Endereço de entrega editado com sucesso!');
                redirect(base_url() . 'index.php/os/editar/'.$this->input->post('idOs'));
    ...
    function editarEndCobranca()
    {
            ...
            $data = array(
                ...
                'rua' => $this->input->post('ruaCB'),
                'numero' => $this->input->post('numeroCB'),
                'bairro' => $this->input->post('bairroCB'),
                'cidade' => $this->input->post('cidadeCB'),
                'estado' => $this->input->post('estadoCB'),
                'cep' => $this->input->post('cepCB')
            );

            if ($this->os_model->edit('endereco_cobranca', $data, 'os_id', $this->input->post('idOs')) == true) {
                $this->session->set_flashdata('success', 'Endereço de cobrança editado com sucesso!');
                redirect(base_url() . 'index.php/os/editar/'.$this->input->post('idOs'));
    ...
    public function faturar()
    {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            if ($this->os_model->add('lancamentos', $data) == true) {
                ...
                $this->session->set_flashdata('success', 'Proposta faturada com sucesso!');
                $json = array('result'=>  true);
                echo json_encode($json);
                die();
            } else {
                $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar faturar Proposta.');
                $json = array('result'=>  false);
                echo json_encode($json);
                die();
            }
