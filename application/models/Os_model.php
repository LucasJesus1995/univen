...
class Os_model extends CI_Model
{
    ...
    function getById($id)
    {
        $this->db->select('os.*, clientes.*,usuarios.nome');
        $this->db->from('os');
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
    ...
    public function getCobranca($id = null)
    {
        $this->db->select('os.*, endereco_cobranca.*');
        $this->db->from('os');
        $this->db->join('endereco_cobranca', 'endereco_cobranca.os_id = os.idOs');
        $this->db->where('os.idOs', $id);
        $this->db->limit(1);
        return $this->db->get()->row();
    }
    ...
    public function autoCompleteUsuario($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label'=>$row['email'].' | Nome: '.$row['nome'],'id'=>$row['idUsuarios']);
            }
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteServico($q)
    {
     ...
     public function autoCompleteRepresentante($q)
    {
